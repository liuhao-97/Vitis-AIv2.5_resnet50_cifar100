...
def quantization(title='optimize',
    ...
    model=resnet50()
    model.load_state_dict(torch.load('resnet50-200-best.pth'))
    ...
    if quant_mode == 'float':
        ...
        if inspect:
            ...
            # start to inspect
            inspector.inspect(quant_model, (input,), device=device,image_format='png')
            sys.exit()
    else:
        # new api
        ####################################################################################
...
if __name__ == "__main__":
    ...
    model_name = 'resnet50'
    file_path = os.path.join(args.model_dir, 'resnet50-200-best.pth')
